...
// GetBytes gets a raw value from a bucket.
func (n *node) GetBytes(bucketName string, key any) ([]byte, error) {
...
// SetBytes sets a raw value into a bucket.
func (n *node) SetBytes(bucketName string, key any, value []byte) error {
	if key == nil {
		return ErrNilParam
	}
...
// Get a value from a bucket
func (n *node) Get(bucketName string, key any, to any) error {
	ref := reflect.ValueOf(to)

	if !ref.IsValid() || ref.Kind() != reflect.Pointer {
		return ErrPtrNeeded
	}
...
// Set a key/value pair into a bucket
func (n *node) Set(bucketName string, key any, value any) error {
	var data []byte
	var err error
	if value != nil {
...
// Delete deletes a key from a bucket
func (n *node) Delete(bucketName string, key any) error {
	id, err := toBytes(key, n.codec)
	if err != nil {
		return err
	}
...
// KeyExists reports the presence of a key in a bucket.
func (n *node) KeyExists(bucketName string, key any) (bool, error) {
	id, err := toBytes(key, n.codec)
	if err != nil {
		return false, err
	}
